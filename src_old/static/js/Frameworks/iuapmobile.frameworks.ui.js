...
(function(global, factory) {
  ...
  _UModal.prototype = {
    ...
    _showModal: function() {
      this.overlay.css('height',100000);
      this.settings.overlay && this.overlay.appendTo($('body')).show();

      var modal = $(this.html).appendTo($('body')),

          modalH = modal.outerHeight(),
          /*wh 兼容原生的头部*/
          wh =this.settings.minHeight ?this.settings.minHeight : summer.getDeviceInfo().screen.height;
         var modalTop=parseInt(this.settings.top) ? parseInt(this.settings.top):0;
        var marginLeft= -modal.outerWidth()/2 ;
            console.log(modal);
      modal.css({'top': (wh - modalH - 20) / 2+modalTop,marginLeft:marginLeft});
    ...
    destroy: function() {
      var that = this;
        that.modal.remove();
        // 避免遮罩闪烁
        that.settings.overlay && that.overlay.remove();
         },
    _init: function() {
...
(function (global, factory) {
    ...
    activeDom.prototype = {
        ...
        _showHtml: function () {
            if(this.settings.inFrame){
                var actionSheet = $(this.content).appendTo($('.um-frame'));
