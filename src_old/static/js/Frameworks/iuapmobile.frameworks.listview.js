	...
	paths : {
		...
		ListView.prototype.init = function(options) {
            ...
            self.config = {
				...
				itemStickEnable:false,//是否已开启置顶事件监听
				changeReadStateEnable:false, //是否已开启读取信息事件监听
			...
			self.opts = $.extend({}, {
				...
				itemStickAnimateTime : 500,
				changeReadStateAnimateTime : 500
			}, options);
		...
		ListView.prototype.on = function(event, callback) {
			...
			switch (event) {
				...
				case "itemDelete":
					...
					break;
				case "itemStick":
					this.itemStick(callback);
					break;
				case "changeReadState":
					this.changeReadState(callback);
					break;
			}
       ...
       /*新增置顶事件*/
	ListView.prototype.itemStick = function(callback) {
		var self = this;
		if ( typeof callback == 'function') {
			this._events.on("itemStick", function(sender, args) {
				var $domSideMenu = args.$target.find('.um-swipe-btns');
				$domSideMenu.removeClass('um-swipe-open');
				callback(sender, args);
			});
			if (!this.config.itemStickEnable) {
				this.$list.delegate('.um-listview-row ' + '.um-stick', 'click', function(e) {
					e.stopPropagation();
					e.preventDefault();
					var $target = $(this).closest('li');
					var rowIndex = $target.index();
					...
					self._events.trigger("itemStick", self, args);
				});
				this.config.itemStickEnable = true;
			}
		}
		return this;
	};
	/*新增阅读状态改变事件*/
	ListView.prototype.changeReadState = function(callback) {
		var self = this;
		if ( typeof callback == 'function') {
			this._events.on("changeReadState", function(sender, args) {
				var $domSideMenu = args.$target.find('.um-swipe-btns');
				$domSideMenu.removeClass('um-swipe-open');
				callback(sender, args);
			});
			if (!this.config.changeReadStateEnable) {
				this.$list.delegate('.um-listview-row ' + '.um-change-read-state', 'click', function(e) {
					e.stopPropagation();
					e.preventDefault();
					var $target = $(this).closest('li');
					var rowIndex = $target.index();
					...
					self._events.trigger("changeReadState", self, args);
				});
				this.config.changeReadStateEnable = true;
			}
		}
		return this;
	};
